...
var MainDigits = 5;
var RotationTime = 35000; // time ms
var RotationDegree = 3180*(RotationTime/35000);
var HitDistance = 20; // hit distance in px
var killTimeout;
var active = false;
...
(() => {
    ...
    CraftGame.Close = function() {
        ...
        $.post("http://urban_crafting/CloseGame", JSON.stringify({}));
    };

    CraftGame.Failed = function() {
        if (active) $.post("http://urban_crafting/CraftingFailed",'{}');
        active = false;
        CraftGame.Close();
    }

    CraftGame.Success = function() {
        if (active) $.post("http://urban_crafting/CraftingSuccess",'{}');
        active = false;
        CraftGame.Close();
    }
