...
function OpenPropertyInventoryMenu(property, owner)
	exports['mythic_progbar']:Progress({
		...
		if not cancelled then
			TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10, "StashOpen", 0.5)
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'room_exit' then
					...
					exports['mythic_progbar']:Progress({
						...
						if not cancelled then
							-- TriggerEvent('97de15da-ecd7-42bb-b562-2bd77f1f6d37')
							TriggerEvent('instance:leave')
							--ExitProperty(CurrentActionData.propertyName)
							Citizen.Wait(850)
							TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 5.0, 'DoorClose', 0.7)
						end
					end)
				end
			end
