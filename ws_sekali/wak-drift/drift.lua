...
function ToggleDrift()
	...
	if driftMode then
		...
		exports['mythic_notify']:SendAlert('error', 'MODE STANDAR AKTIF!', 10000)
	--	DrawNotif("~y~TCS~s~, ~y~ABS~s~, ~y~ESP ~s~is ~g~on~s~!\nVehicle MODE STANDAR AKTIF!")
	else
	...
	--	DrawNotif("~y~TCS~s~, ~y~ABS~s~, ~y~ESP ~s~is ~r~OFF~s~!\nEnjoy MODE DRFIT AKTIF!")
		exports['mythic_notify']:SendAlert('success', 'MODE DRFIT AKTIF!', 10000)
	end
