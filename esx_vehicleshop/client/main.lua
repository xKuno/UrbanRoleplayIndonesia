	...
	ESX.TriggerServerCallback('urban_prime:isPrime', function(prime)
		...
		for i=1, #Categories, 1 do
			...
			for j=1, #categoryVehicles, 1 do
				...
				if prime then
					vehicle.price = vehicle.price * (1 - (Config.primeDiscount / 100))
				end
				...
				if prime then
					table.insert(options, ('%s <span style="color:green;">Diskon:%s</span>'):format(vehicle.name, _U('generic_shopitem', ESX.Math.GroupDigits(vehicle.price))))
				else
...
Citizen.CreateThread(function ()
	while true do
		...
		for k,v in pairs(Config.Zones) do
			local vCoords = vector3(v.Pos.x, v.Pos.y, v.Pos.z)
