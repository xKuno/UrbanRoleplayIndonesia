...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    {label = ('Baju Cartel'), value = 'cartel_wear'},
    ...
    ESX.UI.Menu.Open(
      ...
      if data.current.value == 'cartel_wear' then
        ...
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
          ...
          if skin.sex == 0 then
            TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['seragam_cartel'].male)
          else
            TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['seragam_cartel'].female)
          end
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
		 -- TriggerServerEvent('esx_carteljob:tutupArmory')		
		 -- valididi=false
...
-- RegisterCommand('berimarjun', function(source)
	-- TriggerServerEvent('esx_carteljob:getStockItem', 'marijuana', 1)
-- end)
	...
	if Config.EnablePlayerManagement then
	    ...
	    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'loker', {
		   ...
		   {label = _U('loker_take', _U('blueprint')), item = 'blueprint_clip', type = 'slider', value = 1, min = 1, max = 5},
		    ...
		    ESX.TriggerServerCallback('pri-cartel:buy', function(bought)
				if bought == true then 
				if PlayerData.job.grade > 2 then
				    TriggerServerEvent('pri-cartel:giveItem', data.current.item, data.current.value)
  ...
  ESX.TriggerServerCallback('esx_carteljob:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBillBill', GetPlayerServerId(player), 'society_cartel', _U('fine_total') .. label, amount)
        else
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_carteljob:buy', function(hasEnoughMoney)
          ...
          if hasEnoughMoney then
            ESX.TriggerServerCallback('esx_carteljob:addArmoryWeaponBuy', function()
              OpenBuyWeaponsMenu(station)
            end, data.current.value)
          else
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'cartel' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'menu_armory' then
			...
			-- ESX.TriggerServerCallback('esx_carteljob:checkarmoryuse',function(validsource)
				-- if(validsource==false) then					
					-- TriggerServerEvent('esx_carteljob:bukaArmory')									
					-- valididi=true							
					-- local playerPed = PlayerPedId()
					-- FreezeEntityPosition(playerPed, true)
