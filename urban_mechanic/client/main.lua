...
function OpenLokerMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
	    ...
	    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'loker', {
		    ...
		    align    = 'top-left',
		    elements = {
			    {label = _U('loker_take', _U('fixkit')), item = 'fixkit', type = 'slider', value = 1, min = 1, max = 20},
			    {label = _U('loker_take', _U('repairkit')), item = 'repairkit', type = 'slider', value = 1, min = 1, max = 3}
	    }}, function(data, menu)
		    ESX.TriggerServerCallback('esx_mechanicjob:buy', function(bought)
			    if bought == true then 
...
function OpenMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		elseif data.current.value == 'cloakroom' then
          ...
          if skin.sex == 0 then
            TriggerEvent('skinchanger:loadClothes', skin, Config.seragamwak['seragam_mekanik'].male)
          else
            TriggerEvent('skinchanger:loadClothes', skin, Config.seragamwak['seragam_mekanik'].female)
          end
			end)
...
function OpenMobileMechanicActionsMenu()
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		ESX.TriggerServerCallback('esx_mechanicjob:fixvehicle', function(items)
		    if items == true then
		  	  
		        TriggerServerEvent('esx_mechanicjob:removeItem')
		        ...
		        if DoesEntityExist(vehicle) then
					...
					exports["progressBars"]:taskBar(20000, "Memperbaiki Kendaraan")
			        Citizen.CreateThread(function()
				        Citizen.Wait(3000)
						...
						exports['mythic_notify']:DoHudText('inform', 'Kendaraan telah diperbaiki')
				        isBusy = false
		            end)
		        else
		            ESX.ShowNotification(_U('no_vehicle_nearby'))
...
RegisterNetEvent('esx_mechanicjob:onrepair')
AddEventHandler('esx_mechanicjob:onrepair', function()		
	local playerPed = PlayerPedId()
	local vehicle   = ESX.Game.GetVehicleInDirection()
	local coords    = GetEntityCoords(playerPed)
...
AddEventHandler('esx_mechanicjob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'MechanicActions' then
		...
		CurrentActionMsg  = _U('open_actions')
		CurrentActionData = {}
	elseif zone == 'Loker' then
		CurrentAction     = 'loker_menu_title'
		CurrentActionMsg  = _U('open_actions')
		CurrentActionData = {}
...
RegisterNetEvent('esx_phonee:loaded')
AddEventHandler('esx_phonee:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = _U('mechanic_customer'),
		number     = 'mechanic',
	...
	TriggerEvent('esx_phonee:addSpecialContact', specialContact.name, specialContact.number, specialContact.base64Icon)
end)
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'loker_menu_title' then
				    OpenLokerMenu()
