...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(984.43, 57.75, 75.49),
        dealerHeading = 113.73,
    ...
    [1] = {
        dealerPos = vector3(988.47,59.76,75.49),
        dealerHeading = 57.99,
        tablePos = vector3(987.7701, 60.17849, 74.48293),
        tableHeading = 238.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [2] = {
        dealerPos = vector3(987.27,63.86,75.49),
        dealerHeading = 26.01,
        tablePos = vector3(986.8749, 64.57017, 74.48395),
        tableHeading = 208.0,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [3] = {
        dealerPos = vector3(990.54,58.52,75.49),
        dealerHeading = 240.79,
    ...
    [4] = {
        dealerPos = vector3(989.44,54.0,75.49),
        dealerHeading = 188.42,
    ...
    [5] = {
        dealerPos = vector3(994.29,60.24,75.49),
        dealerHeading = 286.01,
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Tekan ~INPUT_PICKUP~ untuk bermain blackjack")
                    else 
                        drawNativeNotification("Kursi ini sudah terisi.")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Kartumu ["..tostring(currentHand).."] - Kartu Dealer ["..tostring(dealersHand).."]")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "Tujuan Blackjack adalah untuk mengalahkan tangan Dealer tanpa melebihi 21. Permainan ini menggunakan empat kartu dari 52 kartu, yang dikocok di awal setiap tangan. Dealer akan berdiri di atas soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "SAMA:", 255, 255, 255, 255, 0, 0)
    ...
    if closeToCasino then
        ...
        if closestChair == chairId then
            ...
            if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,globalNextCardCount)
                    declineCard()
                    drawNativeNotification("Gagal berdiri / memukul tepat waktu, berdiri.")
                end
            end)
        else 
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Sampai pertandingan berikutnya dimulai ...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        angryILost()
        canExitBlackjack = true
        drawNativeNotification("~r~Kamu kalah !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Kartu Anda Seri !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("~g~Anda menang !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
function setupBlackjackInstructionalScaleform(scaleform)
    ...
    ButtonMessage("Tinggalkan meja") --BACKSPACE
    PopScaleformMovieFunctionVoid()
    ...
    ButtonMessage("Turunkan taruhannya") --Page Down
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(3)
    Button(GetControlInstructionalButton(2, 10, true))
    ButtonMessage("Tingkatkan taruhan") --Page Up
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("Atur Taruhan") --Space
    PopScaleformMovieFunctionVoid()  
