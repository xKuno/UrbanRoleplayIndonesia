...
Citizen.CreateThread(function()
	while ESX == nil do
		TriggerEvent('esx:getjeavusSharedjeavusObjectjeavus', function(obj) ESX = obj end)
		Citizen.Wait(0)
	end
end)
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 1.0 then
				DrawText3Ds(sitCoords, "E - Duduk", 0.35)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
				DrawText3Ds(pickupCoords, "E - Ambil", 0.35)
...
DrawText3Ds = function(coords, text, scale)
	...
	DrawRect(_x, _y + 0.0150, 0.030 + factor, 0.025, 41, 11, 41, 68)
end
