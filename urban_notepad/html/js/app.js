...
document.onkeyup = function (data) {
    if (data.which == 27) { // Escape key
        $.post('http://urban_notepad/escape', JSON.stringify({}));
        if (notepadreader == true) {
            var x = document.getElementById("p1").value;
            $.post('http://urban_notepad/updating', JSON.stringify({ text: x}));
            $("#main").fadeOut();
...
function dropNotepad() {
    var x = document.getElementById("p1").value;
    if (x == ""){
        $.post('http://urban_notepad/droppingEmpty', JSON.stringify({}));
    }else {
        $.post('http://urban_notepad/dropping', JSON.stringify({ text: x}));
        $("#main").fadeOut();
        document.getElementById("p1").value = "";
        $("#main").css('display', 'none');
    }   
