...
RegisterNUICallback('escape', function(data, cb)
    ...
    TriggerEvent("urban_notepad:CloseNotepad")
end)
...
RegisterNetEvent("urban_notepad:OpenNotepadGui")
AddEventHandler("urban_notepad:OpenNotepadGui", function()
    if not isUiOpen then
        openGui()
    end
end)

RegisterNetEvent('urban_notepad:closehtmldos')
AddEventHandler('urban_notepad:closehtmldos', function()
    TriggerEvent("urban_notepad:CloseNotepad")
end)

RegisterNetEvent("urban_notepad:CloseNotepad")
AddEventHandler("urban_notepad:CloseNotepad", function()
    SendNUIMessage({
        action = 'closeNotepad'
    })
...
RegisterNetEvent('urban_notepad:note')
AddEventHandler('urban_notepad:note', function()
    ...
    if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
        ...
        if ( IsEntityPlayingAnim( player, ad, "base", 3 ) ) then 
            ...
            AttachEntityToEntity(prop, player, GetPedBoneIndex(player, 18905), 0.1, 0.02, 0.05, 10.0, 0.0, 0.0, true, true, false, true, 1, true) -- urban_notepadpad
...
RegisterNetEvent('urban_notepad:updateNotes')
AddEventHandler('urban_notepad:updateNotes', function(serverNotesPassed)
...
function openGuiRead(text)
  ...
  if GetPedInVehicleSeat(veh, -1) ~= GetPlayerPed(-1) then
        SetPlayerControl(PlayerId(), 0, 0)
        TriggerEvent("urban_notepad:note")
        isUiOpen = true
    ...
    while true do
        ...
        if #TestLocalTable == 0 then
            ...
            if distance < 2.0 then
                ...
                DrawText3Ds(TestLocalTable[closestNoteId]["x"],TestLocalTable[closestNoteId]["y"],TestLocalTable[closestNoteId]["z"]-0.4, "[~g~E~s~] Membaca | [~g~G~s~] Buang")
