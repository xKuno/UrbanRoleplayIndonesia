...
AddEventHandler('instance:onEnter', function(instance)
    if instance.type == 'motelroom' then
        ...
        TriggerServerEvent('InteractSound_SV:PlayOnSource', 'DoorClose', 0.9)
	end
end)
...
AddEventHandler('lsrp-motels:roomOptions', function(room, motel)
    ...
    local elements = {
        ...
        { label = 'Berhenti Menyewa', value = 'cancel' }
    }

    ESX.TriggerServerCallback('lsrp-motels:checkExpired', function(data)
        if data == true then
            table.insert(elements, {label = 'Bayar rental sekarang!', value = 'pay_rent'})
        else
            table.insert(elements, {label = 'Kamu dapat membayar rental '..data..' jam lagi', value = 'nothing'})
        end
    end)
    ...
    function(data, entry)
		...
		entry.close()
           hasChest = true
			TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 5.0, 'DoorOpen', 0.7)
        ...
        elseif value == 'pay_rent' then
            entry.close()
            TriggerServerEvent('lsrp-motels:payRent')
        end
...
AddEventHandler('lsrp-motels:roomMenu', function(room, motel)
    ...
    for k,v in pairs(Config.Zones) do
     ...
     --   table.insert(options, {label = 'Open inventory', value = 'inventory'})
        table.insert(options, {label = 'Undang Teman', value = 'inviteplayer'})
        end
    ...
    function(data, menu)
       ...
       elseif value == 'leaveroom' then
        ...
        if roomOwner == owner then
		 TriggerServerEvent('InteractSound_SV:PlayOnSource', 'StashOpen', 0.9)
                    TriggerEvent("mythic_progbar:client:progress", {
                        name = "inventory",
                        duration = 1500,
                        label = "Membuka inventory motel",
                        useWhileDead = false,
                        canCancel = true,
                        controlDisables = {
                                disableMovement = true,
            ...
            TriggerClientEvent('esx:showNotification', 'Akses hanya untuk pemilik!')  
        end
        elseif value == 'inviteplayer' then
            ...
            if playersInArea ~= nil then
                ...
                table.insert(elements, {label = 'Tidak ada yang di pintu'})
            end
...
AddEventHandler('lsrp-motels:rentRoom', function(room)
    ...
    ESX.TriggerServerCallback('lsrp-motels:checkOwned', function(isOwned)
        if not isOwned then
            ...
            ESX.ShowNotification('Kamar ini sudah dimiliki seseorang!')
        end
    end, room)
end)
...
function roomMarkers()
   ...
   --[[ for k,v in pairs(Config.Zones) do
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            if roomOwner == playerIdent then
            DrawText3D(v.Inventory.x, v.Inventory.y, v.Inventory.z + 0.35, 'Tekan [~g~E~s~] untuk membuka lemari baju.')
                if IsControlJustReleased(0, Keys['E']) then
                    PlayerDressings()
                end
            end
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            if roomOwner == playerIdent then
            DrawText3D(v.BedStash.x, v.BedStash.y, v.BedStash.z + 0.1, 'Tekan [~g~E~s~] untuk membuka penyimpanan')
                if IsControlJustReleased(0, Keys['E']) then
                    OpenStash()
                end
            end
        ...
        for k,v in pairs(Config.Zones) do
            ...
            if (distance < v.Boundries) then
                for km,vm in pairs(v.Rooms) do
                    distance = GetDistanceBetweenCoords(coords, vm.entry.x, vm.entry.y, vm.entry.z, true)
                    if (distance < 1.0) then
                        DrawText3D(vm.entry.x, vm.entry.y, vm.entry.z + 0.35, 'Tekan [~g~E~s~] masuk kamar ~b~'..vm.number..' ~w~menyewa seharga ~b~$'..Config.PriceRental)
                        if IsControlJustReleased(0, Keys['E']) then
