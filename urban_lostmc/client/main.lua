...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    {label = ('Baju biker'), value = 'biker_wear'},
    ...
    ESX.UI.Menu.Open(
      ...
      if data.current.value == 'biker_wear' then
        ...
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
          ...
          if skin.sex == 0 then
            TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['seragam_biker'].male)
          else
            TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['seragam_biker'].female)
          end
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
		 -- TriggerServerEvent('esx_bikerjob:tutupArmory')		
		 -- valididi=false
...
-- RegisterCommand('berimarjun', function(source)
	-- TriggerServerEvent('esx_bikerjob:getStockItem', 'marijuana', 1)
-- end)
...
function OpenLokerMenu()
	if Config.EnablePlayerManagement then
	    ...
	    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'loker', {
		    ...
		    ESX.TriggerServerCallback('pri-biker:buy', function(bought)
				if bought == true then 
				if PlayerData.job.grade > 1 then
				    TriggerServerEvent('pri-biker:giveItem', data.current.item, data.current.value)
  ...
  ESX.TriggerServerCallback('esx_bikerjob:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBillBill', GetPlayerServerId(player), 'society_biker', _U('fine_total') .. label, amount)
        else
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_bikerjob:buy', function(hasEnoughMoney)
          ...
          if hasEnoughMoney then
            ESX.TriggerServerCallback('esx_bikerjob:addArmoryWeaponBuy', function()
              OpenBuyWeaponsMenu(station)
            end, data.current.value)
          else
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'biker' then
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'biker' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'menu_armory' then
			...
			-- ESX.TriggerServerCallback('esx_bikerjob:checkarmoryuse',function(validsource)
				-- if(validsource==false) then					
					-- TriggerServerEvent('esx_bikerjob:bukaArmory')									
					-- valididi=true							
					-- local playerPed = PlayerPedId()
					-- FreezeEntityPosition(playerPed, true)
