local Keys = {
  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 then
			  DrawText3D(x,y,z, '[~g~E~s~] Periksa', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'Luka disini', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	 ...
	 table.insert(elements, {label = 'Penyebab Pingsan', value = 'deathcause'})
	  table.insert(elements, {label = 'Luka Terjadi', value = 'damage'})
