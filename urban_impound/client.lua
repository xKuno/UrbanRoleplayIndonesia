...
function ActivateBlips()
    ...
    AddTextComponentString("Impound Company")
...
AddEventHandler('HRP:Impound:VehicleUnimpounded', function (data, index)
	...
	_ESX.ShowNotification("Kendaraan kamu dengan plat: " .. data.plate .. " Sudah keluar dari sitaan!")
...
AddEventHandler('HRP:Impound:CannotUnimpound', function ()
	exports['mythic_notify']:DoHudText('inform', 'Tidak cukup uang!')
end)
...
function ShowImpoundMenu (action)
	...
	if (IsPedInAnyVehicle(GetPlayerPed(PlayerId()))) then
	 exports['mythic_notify']:DoHudText('inform', 'Keluar kendaraan untuk menyita')
		return
	end
	...
	if (vehicle ~= nil) then
		...
		if(_VehicleAndOwner == nil) then
			exports['mythic_notify']:DoHudText('error', 'Pemilik kendaraan tidak dikenal, tidak dapat menyita')
			return
		end
...
RegisterNUICallback('impound', function(data, cb)
	...
	if (veh.plate:gsub("%s+", "") ~= data.plate:gsub("%s+", "")) then
		exports['mythic_notify']:DoHudText('error', 'Kendaraan yang diproses, dan kendaraan terdekat tidak cocok')
		return
	end
