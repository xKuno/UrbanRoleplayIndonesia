$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://urban_impound/escape', JSON.stringify({}));
		}
	};
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://urban_impound/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://urban_impound/unimpound', JSON.stringify(plate));
	});

	$('table').on('click', '.unlock', function () {
		var plate = $(this).parent().parent().find('#plate').text();
		$.post('http://urban_impound/unlock', JSON.stringify(plate));
	});
	...
	$('#cancel, #exit').click(function (event) {
		$.post('http://urban_impound/escape', null);
	});
