...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    {label = ('Baju mafia'), value = 'mafia_wear'},
    ...
    ESX.UI.Menu.Open(
      ...
      if data.current.value == 'mafia_wear' then
        ...
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
          ...
          if skin.sex == 0 then
            TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['seragam_mafia'].male)
          else
            TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['seragam_mafia'].female)
          end
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
		 -- TriggerServerEvent('esx_mafiajob:tutupArmory')		
		 -- valididi=false
...
-- RegisterCommand('berimarjun', function(source)
	-- TriggerServerEvent('esx_mafiajob:getStockItem', 'marijuana', 1)
-- end)
...
function OpenLokerMenu()
	if Config.EnablePlayerManagement then
	    ...
	    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'loker', {
		    ...
		    ESX.TriggerServerCallback('pri-mafia:buy', function(bought)
				if bought == true then 
				if PlayerData.job.grade > 1 then
				    TriggerServerEvent('pri-mafia:giveItem', data.current.item, data.current.value)
  ...
  ESX.TriggerServerCallback('esx_mafiajob:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBillBill', GetPlayerServerId(player), 'society_mafia', _U('fine_total') .. label, amount)
        else
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_mafiajob:buy', function(hasEnoughMoney)
          ...
          if hasEnoughMoney then
            ESX.TriggerServerCallback('esx_mafiajob:addArmoryWeaponBuy', function()
              OpenBuyWeaponsMenu(station)
            end, data.current.value)
          else
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'mafia' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'menu_armory' then
			...
			-- ESX.TriggerServerCallback('esx_mafiajob:checkarmoryuse',function(validsource)
				-- if(validsource==false) then					
					-- TriggerServerEvent('esx_mafiajob:bukaArmory')									
					-- valididi=true							
					-- local playerPed = PlayerPedId()
					-- FreezeEntityPosition(playerPed, true)
